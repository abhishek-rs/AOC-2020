...
const day20A = async () => {
  ...
  tileBorders.forEach((val, key, map) => {
    if (Object.keys(val).length === 2) {
      corners.push({ [key]: val });
    }
  });

  let resultImage = new Array(120);
  let emptyRow = new Array(120);
  let resultImageGuide = new Array(12);
  let emptyRowGuide = new Array(12);
  emptyRow.fill(null);
  emptyRowGuide.fill(null);
  resultImage.fill(emptyRow);
  resultImageGuide.fill(emptyRowGuide);

  fillImage(resultImage, resultImageGuide, tiles, tileBorders, corners[0]);
...
const fillImage = (result, guide, tiles, borders, startTile) => {
  for (let i = 0; i < 12; i++) {
    for (let j = 0; j < 12; j++) {
      if (i === 0 && j === 0) {
        guide[i][j] = startTile;
        let currentTile = tiles.get(Number(Object.keys(startTile)[0]));
        insertTile(result, currentTile, i, j);
        continue;
      }
      let prevTile = j === 0 ? guide[i - 1][j] : guide[i][j - 1];
    }
  }
};
