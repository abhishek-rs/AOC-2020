...
const getAllVisibleAdjacents = (rowNo, colNo, maxRows, maxCols, chart) => {
  ...
  for (i = rowNo - 1; i <= rowNo + 1; i++) {
    for (j = colNo - 1; j <= colNo + 1; j++) {
      if (
        ...
        if (chart[i][j] === '.') {
          // If ground, look further till you reach occupied/empty
          let [xDir, yDir] = [i - rowNo, j - colNo];
          for (
            k = i, l = j;
            k < maxRows && l < maxCols && k > -1 && l > -1;
            k += xDir, l += yDir
          ) {
            if (chart[k][l] !== '.') {
              possibleAdjacents.push([k, l]);
              break;
            }
          }
        } else {
...
const assignSeats = (inputChart) => {
  ...
  seatingChart.forEach((row, x) => {
    for (y = 0; y < row.length; y++) {
      ...
      const adjacentSeats = getAllVisibleAdjacents(
        x,
        y,
        inputChart.length,
        row.length,
      ...
      if (row[y] === 'L' && noOfOccupiedAdjacents === 0) {
        row[y] = '#';
      } else if (row[y] === '#' && noOfOccupiedAdjacents >= 5) {
        row[y] = 'L';
      } else {
        continue;
      }
